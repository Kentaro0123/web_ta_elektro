      ...
      <div class="row ">
        @foreach ($skripsi as $item)
          ...
          <h4> {{ $item->riwayat_mahasiswa->user->nama }}</h4>
              ...
              <a class="card bg-success text-white btn">
                ...
                <div class="card-body">
                  ...
                  <?php
                    $array_tmp=[]; 
                  ?>
                      @foreach ($item->riwayat_mahasiswa->topik->user->penilaian_akhir as $nilai)
                  ...
                  <?php 
                    ...
                    foreach ($array_tmp as $key => $value) {
                        foreach ($format_penilaian as $fp) {
                          if($fp->id ==  $key){
                            $array_tmp[$key] = ($fp->bobot_nilai*$array_tmp[$key] / 100)/ sizeof($fp->format_sub_penilaian_akhir);
              ...
              <div class="col-md-4">        
                ...
                <a class="card text-white btn" style="background-color:rgb(139, 139, 38)">
                  ...
                  <div class="card-body">
                    ...
                    @foreach ($item1->user->penilaian_akhir as $nilai)
                      @if ($nilai->skripsi_id == $item->id)
                        @foreach ($format_penilaian as $fp)
                          @if ($fp->id == $nilai->format_penilaian_id)
                              <?php 
            ...
            @foreach ($item->jadwal_sidang_akhir->dosen_sidang_akhir as $item2)
              <div class="col-md-4">        
                ...
                <a class="card text-white bg-danger btn" >
                  ...
                  <div class="card-body">
                    ...
                    @foreach ($item2->user->penilaian_akhir as $nilai)
                      @if ($nilai->skripsi_id == $item->id)
                        @foreach ($format_penilaian as $fp)
                          @if ($fp->id == $nilai->format_penilaian_id)
                              <?php 
            ...
            @endforeach
          <p></p>
              @if ($item->lulus_skripsi == 1)
              
                <a type="button" href="cancel_lulus_skripsi/{{$item->id}}" class="btn btn-danger">Cancel</a>
                  
              @else
                <a type="button" href="lulus_skripsi/{{$item->id}}" class="btn btn-success">Lulus</a>
